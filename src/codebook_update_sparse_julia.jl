...
function update_codebooks_spgl1_julia(
  ...
  if V println("Doing SPGL1 codebook update... "); st=time(); end
  ...
  for ii = 1:d
    x, r, g, info = GenSPGL.spgl1(C, Xt[:,ii], tau=Float64(tau), options = opts)
    K[ii,:] = x
  end

  # GenSPGL.spgl1(full(C), Xt[:,1], tau=tau)
...
function update_codebooks_spgl1_threshold_julia(
  X::Matrix{Float32}, # d-by-n matrix to update codebooks on
  B::Matrix{Int16},   # m-by-n matrix. X encoded
  h::Integer,         # number of entries per codebook
  tau::Float32,       # \tau argument in SPGL1. Sets sparsity
  ...
  C = update_codebooks_spgl1_julia( X, B, h, tau, prevC, spgl1_path, V );
  K = hcat( C... );
